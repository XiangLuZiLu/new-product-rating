...
export function getStorage(env) {
  ...
  if (['edgekv', 'edgeone-kv', 'edgeone_kv', 'edgeone'].includes(driver)) return createEdgeOneKVStorage(env);
...
function createEdgeOneKVStorage(env) {
  ...
  if (typeof EdgeKVClass === 'function') {
    const namespace = String(env.ESA_KV_NAMESPACE || env.EDGEONE_KV_NAMESPACE || env.KV_NAMESPACE || 'product_review').trim();
    if (!namespace) throw new Error('当前使用边缘 KV 存储，但未配置 ESA_KV_NAMESPACE / EDGEONE_KV_NAMESPACE。');
  ...
  throw new Error(`当前使用边缘 KV 存储，但未找到可用的 KV。ESA Pages 请配置 ESA_KV_NAMESPACE；EdgeOne Pages 请绑定 KV 对象（建议变量名 ${bindingName}）。`);
}
