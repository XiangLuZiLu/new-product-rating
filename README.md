...
本版本修复阿里云 ESA Pages 后台登录接口 401 时前端误提示“请先登录”的问题，并对 ADMIN_PASSWORD 做了空格兼容。
...
## 同时支持 Cloudflare Pages 和阿里云 ESA Pages

本版本增加了阿里云 ESA Pages 入口：`aliyun/entry.js`，并提供 `esa.jsonc`。Cloudflare Pages 仍然使用 `functions/` 目录自动识别 Pages Functions；阿里云 ESA Pages 使用 `esa.jsonc` 中的 `entry` 和 `assets.directory`。

- Cloudflare 推荐：`STORAGE_DRIVER=d1`，图片可用 R2 或后台配置 S3/OSS。
- 阿里云推荐：`STORAGE_DRIVER=edgekv`，`EDGEKV_NAMESPACE=product_review`，图片用后台配置的阿里云 OSS / 七牛云 / COS。

详细步骤见：`docs/ALIYUN_ESA_PAGES_SETUP.md`。


## 阿里云 ESA Pages 注意事项

- 阿里云默认后台入口建议使用 `/admin`。
- 阿里云环境变量建议：`STORAGE_DRIVER=edgekv`、`EDGEKV_NAMESPACE=product_review`、`ADMIN_PATH=admin`。
- 如果环境变量暂时未注入，代码会在 `*.aliyun-esa.net` 默认域名下自动兜底为 EdgeKV，避免误提示 Cloudflare D1 未绑定。
- Cloudflare 仍可继续使用 `STORAGE_DRIVER=d1` + D1 Binding。

## 阿里云 ESA Pages 环境变量说明

阿里云 ESA Pages 控制台里的环境变量主要在构建过程中通过 `process.env` 提供。本项目会在 `npm run build` 时把必要变量写入 `aliyun/env.generated.js`，供边缘函数运行时读取。
