...
适合搭建到 Cloudflare Pages 的新品评审评分系统，支持手机端滑动评分、每个款式独立一页、评分份数后台自定义、历史数据查询编辑、导出 CSV、后台入口自定义，以及图片/数据分离存储。

## 功能

- 手机端适配：可左右滑动切换款式，每款单独一页评分。
