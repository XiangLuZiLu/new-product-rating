...
export function imageSettingsFromEnv(env = {}) {
  return normalizeImageSettings({
    ...
    public_image_path_prefix: env.PUBLIC_IMAGE_PATH_PREFIX || env.IMAGE_PUBLIC_PATH_PREFIX || env.S3_PUBLIC_PATH_PREFIX || env.OSS_PUBLIC_PATH_PREFIX || '',
...
export function normalizeImageSettings(value = {}, previous = DEFAULT_IMAGE_SETTINGS) {
  ...
  return {
    ...
    public_image_path_prefix: String(input.public_image_path_prefix ?? input.public_path_prefix ?? input.PUBLIC_IMAGE_PATH_PREFIX ?? input.S3_PUBLIC_PATH_PREFIX ?? prev.public_image_path_prefix ?? '').trim().replace(/^\/+|\/+$/g, ''),
...
function normalizeStyleIdValue(value, label = '款式ID') {
  ...
  // ESA EdgeKV 中款式 ID 使用 UUID 字符串，不能按 D1 的数字 ID 校验。
  return raw;
}
...
function defaultReviewLinkName(styleCount, value = new Date()) {
...
function normalizeReviewLinkPayload(payload = {}, availableStyles = []) {
  ...
  return {
    name: String(payload.name || '').trim().slice(0, 80) || defaultReviewLinkName(style_ids.length),
    style_ids,
...
export function getStorage(env) {
  ...
  if (['edgekv', 'esa-kv', 'esa_kv', 'esa'].includes(driver)) return createEsaEdgeKVStorage(env);
...
function createEsaEdgeKVStorage(env) {
  const cleanKey = (key) => String(key || '')
    .replace(/[^a-zA-Z0-9_]+/g, '_')
    .replace(/^_+|_+$/g, '')
    .slice(0, 512) || 'empty_key';

  // Alibaba Cloud ESA exposes EdgeKV as a built-in global constructor.
  // Keep env.EdgeKV as a local-test fallback only; no EdgeOne/Cloudflare bindings are read here.
  const EdgeKVClass = globalThis.EdgeKV || env.EdgeKV;
  if (typeof EdgeKVClass !== 'function') {
    throw new Error('ESA Pages 未提供 EdgeKV 运行时。请确认项目部署在 ESA Functions/Pages，并已开通边缘 KV。');
  }

  const namespace = String(env.ESA_KV_NAMESPACE || env.KV_NAMESPACE || 'product_review').trim();
